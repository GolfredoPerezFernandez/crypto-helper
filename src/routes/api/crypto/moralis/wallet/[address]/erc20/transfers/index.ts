...
import { fetchMoralisWalletAddressErc20Transfers } from "~/server/crypto-helper/moralis-api";
import { isEvmAddress } from "~/server/crypto-helper/market-queries";
import { verifyAuth } from "~/utils/auth";

/** GET /{address}/erc20/transfers — ERC20 token transfers for wallet. Session required. */
export const onGet: RequestHandler = async (ev) => {
  ...
  const limit = Math.min(100, Math.max(1, Number(ev.query.get("limit")) || 50));
  ...
  const r = await fetchMoralisWalletAddressErc20Transfers(address, chain, {
    limit,
    cursor,
    order,
