...
export default component$(() => {
  ...
  const trimmed = useComputed$(() => wallet.value.trim());
  ...
  const copy = useComputed$(() => {
    ...
    return {
      ...
      title: tr("dashboard.solanaNftLandingTitle@@Solana NFTs"),
      intro: tr(
        "dashboard.solanaNftLandingIntro@@Enter a wallet address to see NFT holdings in the wallet view.",
      ),
  ...
  const openWallet = $(() => {
    if (!isSolanaWalletAddress(wallet.value.trim())) return;
    const a = wallet.value.trim();
      ...
      <section class="rounded-xl border border-[#04E6E6]/25 bg-[#001a1c]/80 p-4 space-y-3" aria-label={copy.value.title}>
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <input
            ...
            value={wallet.value}
            onInput$={(e) => {
              wallet.value = (e.target as HTMLInputElement).value;
            }}
          />
          <select
